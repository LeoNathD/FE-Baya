...
onMounted(async () => {
    ...
    if (route.query.parent_id) {
        ...
        productCategory.value.parent_id = route.query.parent_id
            ...
            <div class="group/errorState flex flex-col gap-2 w-1/2" :class="{ 'invalid': error?.description}">
                <label
                    ...
                    <div class="flex h-full pr-4 pt-2 border-r-[1.5px] border-custom-border ">
                        <img src="@/assets/images/icons/document-text-grey.svg" class="flex size-6 shrink-0"
                            alt="icon">
                    </div>
                ...
                <span
                    class="font-semibold text-lg text-custom-red leading-none group-[&.invalid]/errorState:block" v-if="error?.description">
